...
EXPECTED_OPS = {
    ...
    "memory.scatter_req_to_token_pool": {"triton"},
...
class TestKernelsNamespace(unittest.TestCase):
    ...
    def test_every_in_tree_spec_target_names_a_symbol_that_exists(self):
        # A well-formed target string is not enough: a renamed or deleted symbol
        # leaves a stale entry that only fails when someone resolves the op.
        # Scoped to in-tree sglang.kernels.ops targets: sgl_kernel / jit_kernel
        # symbols live in a wheel or are JIT-built, so they are not statically
        # checkable here and need no backend installed to run this lane.
        # Checked by parsing the source, so the namespace stays metadata-only
        # (see test_import_does_not_load_kernel_backends).
        checked = 0
        for spec in self.K.registry.all_specs():
            ...
            if sub_attr:
                ...
                # None => head is not a class statement (e.g. a module-level
                # BaseFusedOp instance), whose attributes cannot be seen here.
